...
void WriteLog(LPCSTR fmt, ...)
{
    ...
    CStringA strA = "[detour]";
    va_list ap;
    va_start(ap, fmt);
    strA.AppendFormatV(fmt, ap);
    va_end(ap);

    ::OutputDebugStringA(strA.GetString());
    printf("%s", strA.GetString());
}
...
static PDETOUR_TRAMPOLINE detour_alloc_trampoline(PBYTE pbTarget)
{
    ...
    found_region:
        size_t nSizeRegion = sizeof(DETOUR_REGION);
        size_t nSizeRegionFree = sizeof(DETOUR_TRAMPOLINE);
    ...
    for (pbTry = pbTarget; pbTry > (PBYTE)pLo;) {
        ...
        if (pbTry >= (PBYTE)(ULONG_PTR)0x70000000 &&
            ...
            int iRnd = rand() % 0x500 + 1;
            iRnd *= DETOUR_REGION_SIZE;
            pbTry = (PBYTE)(ULONG_PTR)(0x70000000 - iRnd - DETOUR_REGION_SIZE);
        }
    ...
    for (pbTry = pbTarget; pbTry < (PBYTE)pHi;) {
        ...
        if (pbTry >= (PBYTE)(ULONG_PTR)0x70000000 &&
            ...
            int iRnd = rand() % 0x500 + 1;
            iRnd *= DETOUR_REGION_SIZE;
            pbTry = (PBYTE)(ULONG_PTR)(0x80000000 + iRnd + DETOUR_REGION_SIZE);
        }
        ZeroMemory(&mbi, sizeof(mbi));
        if (!VirtualQuery(pbTry, &mbi, sizeof(mbi))) {
            break;
